...
module GenericRegexp = struct
  type ('a, 'b) regexp =
    ('a, 'b) skeleton HashCons.cell

  and ('a, 'b) skeleton =
    | EEpsilon
  | EToken of 'a
  | ECat of ('a, 'b) regexp * ('a, 'b) regexp
  | EStar of ('a, 'b) regexp
  | EDisj of ('a, 'b) regexp list
  | EConj of ('a, 'b) regexp list
  | ENeg of ('a, 'b) regexp
  | EOutput of 'b
...
let skeleton = HashCons.data
    ...
    let equal token_equal output_equal s1 s2 =
    ...
    let hash token_hash output_hash s =
...
module type REGEXPS = sig
  type t
  type output_t
  type regexp = (t, output_t) GenericRegexp.regexp
  type skeleton = (t, output_t) GenericRegexp.skeleton
...
module Regexp(T : BASE_TOKENS)(O : OUTPUT) : (REGEXPS with type t = T.t and type output_t = O.t) = struct
  type t = T.t
  type output_t = O.t
  type skeleton = (t, output_t) GenericRegexp.skeleton
  type regexp = (t, output_t) GenericRegexp.regexp
...
let make : skeleton -> regexp =
  let module H = HashCons.ForHashedType(struct
    type t = skeleton
    let equal = GenericRegexp.equal T.equal O.equal
    let hash = GenericRegexp.hash T.hash O.hash
  end)
  in H.make
...
module RESyntax (Token : BASE_TOKENS)(O : OUTPUT)(Regexp : REGEXPS with type t = Token.t and type output_t = O.t) = struct
...
module Eval (Token : TOKENS)(O : OUTPUT)(Regexp : REGEXPS with type t = Token.t and type output_t = O.t) = struct

module Syntax = RESyntax(Token)(O)(Regexp)
open Syntax
...
let extract_output e = match skeleton e with
    EOutput x -> Some x | _ -> None

let rec exec (a : dfa) (q : state) (input : input) (i : int) =
  let { decode; transition; _ } = a in
  let decq = decode q in
  if nullable decq then
    (* [q] is final. Accept. *)
    Some (input, i, extract_output decq)
  else
    match input() with
    | Seq.Nil ->
        (* No more input. Fail. *)
