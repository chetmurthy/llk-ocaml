...
let skeleton = HashCons.data
    ...
    let equal token_equal s1 s2 =
      ...
      | EAny, EAny ->
          true
      | EExcept l1, EExcept l2 ->
         List.length l1 = List.length l2 &&
           List.for_all2 token_equal l1 l2
      | EToken c1, EToken c2 ->
          token_equal c1 c2
    ...
    let hash token_hash s =
      ...
      | EAny ->
          1
      | EExcept l ->
         List.fold_left (fun h t -> h %% (token_hash t)) 2 l
      | EToken c ->
          3 %% token_hash c
      | ECat (e1, e2) ->
          4 %% hash e1 %% hash e2
      ...
      | EConj es ->
          List.fold_left (fun h e -> h %% hash e) 7 es
      | ENeg e ->
          8 %% hash e
...
let except l : regexp =
  make (EExcept (List.sort_uniq Token.compare l))
...
let rec print0 b e =
  ...
  | EAny ->
      bprintf b "_"
  | EExcept l ->
     bprintf b "[^ %s]" (String.concat " " (List.map Token.print l))
  | EToken c ->
...
let reverse re =
  let rec rerec re = match skeleton re with
    | EEpsilon | EAny | EExcept _ | EToken _ -> re
...
let rec exec (a : dfa) (q : state) (input : input) (i : int) =
  let { decode; transition; _ } = a in
  match final_state (decode q) with
    (* [q] is final. Accept. *)
    Some output ->
    Some (input, output, i)
  | None ->
...
let export { n; init; decode; transition } =
  ...
  for i = 0 to n-1 do
    ...
    acc := (i, rex, final i, List.rev !transacc) :: !acc
  done ;
  (init, decode init, List.rev !acc)
